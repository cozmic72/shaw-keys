...
The library is an **ES module graph**: `shaw-keys.js` imports its two siblings, so a host
...
- **A host must serve it over `http(s)://`.** Module scripts are fetched with CORS and `file://`
  ...
  loading it off disk.
- **The `window.ShawKeys` / `CustomLayouts` / `LayoutEditor` globals are a transition
...
`src/shaw-keys`), and by `shave`. Two things follow, and neither is visible from inside this
checkout:
